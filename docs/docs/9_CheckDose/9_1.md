...
D95%
```csharp
double d_95 = plan.GetDoseAtVolume(structure, 95.0, VolumePresentation.Relative, DoseValuePresentation.Absolute).Dose;
```

D1cc
```csharp
double d_1cc = plan.GetDoseAtVolume(structure, 1.0, VolumePresentation.Absolute, DoseValuePresentation.Absolute).Dose;
```
...
foreach (var structure in plan.StructureSet.Structures)
{
    ...
    {
        // D98%
        double d_98 = plan.GetDoseAtVolume(structure, 98.0, VolumePresentation.Relative, DoseValuePresentation.Absolute).Dose;
        oText += "(" + structure.Id + ") D98%:" + string.Format("{0:f2}", d_98) + " " + plan.TotalDose.UnitAsString + "\n";
    }
    ...
    if (structure.Id.IndexOf("PTV") >= 0 && structure.DicomType == "PTV")
    {
        ...
        // D95%
        double d_95 = plan.GetDoseAtVolume(structure, 95.0, VolumePresentation.Relative, DoseValuePresentation.Absolute).Dose;
        oText += "(" + structure.Id + ") D95%:" + string.Format("{0:f2}", d_95) + " " + plan.TotalDose.UnitAsString + "\n";
        ...
        // HI 
        // (D2% - D98%) / D50% 
        double d_2 = plan.GetDoseAtVolume(structure, 2.0, VolumePresentation.Relative, DoseValuePresentation.Absolute).Dose;
        double d_98 = plan.GetDoseAtVolume(structure, 98.0, VolumePresentation.Relative, DoseValuePresentation.Absolute).Dose;
        double d_50 = plan.GetDoseAtVolume(structure, 50.0, VolumePresentation.Relative, DoseValuePresentation.Absolute).Dose;
        double homogeneityIndex_ICRU83 = (d_2 - d_98)/d_50;
...
foreach (var structure in plan.StructureSet.Structures)
{
    ...
    if (str_lowercase.IndexOf("brain") >= 0 && str_lowercase.IndexOf("stem") >= 0)
    {
        ...
        if(d_1cc.Dose < 59.0)
        {
            oText += "(" + structure.Id + ") D1cc:" + string.Format("{0:f2}", d_1cc) + " " + plan.TotalDose.UnitAsString + "\n";
        }
        else
        {
            oText += "WARNING!! (" + structure.Id + ") D1cc:" + string.Format("{0:f2}", d_1cc) + " " + plan.TotalDose.UnitAsString + " (QUANTEC: < 59.0)\n";
        }
    }
} 
```
...
```csharp
    ...
    if(plan.StructureSet.Structures.Count(s => s.Id.IndexOf("CTV") > 0 && s.DicomType == "CTV") > 0)
    {
        var structure = plan.StructureSet.Structures.Where(s => s.Id.IndexOf("CTV") > 0 && s.DicomType == "CTV");
    ...
    if(plan.StructureSet.Structures.Count(s => s.Id.IndexOf("PTV") > 0 && s.DicomType == "PTV") > 0)
    {

        var structure = plan.StructureSet.Structures.Where(s => s.Id.IndexOf("PTV") > 0 && s.DicomType == "PTV");
    ...
    // Lung
    // V20Gy
    if (plan.StructureSet.Structures.Count(s => s.Id.ToLower().IndexOf("lung") > 0)
    {
        var structure = plan.StructureSet.Structures.Where(s => s.Id.ToLower().IndexOf("lung") > 0);
    ...
    // Brainstem
    // D1cc
    if (plan.StructureSet.Structures.Count(s => s.Id.ToLower().IndexOf("brain") > 0 && s.Id.ToLower().IndexOf("stem") > 0)
    {

        var structure = plan.StructureSet.Structures.Where(s => s.Id.ToLower().IndexOf("brain") > 0 && s.Id.ToLower().IndexOf("stem") > 0);
